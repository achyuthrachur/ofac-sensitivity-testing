const TIERS = [
  ...
  {
    label: 'MEDIUM',
    badge: 'bg-yellow-900/60 text-yellow-200 border-yellow-700/50',
...
export function TierLegend() {
  return (
    <div className="flex flex-wrap gap-3 py-3 px-4 mb-3 bg-white/5 rounded-lg border border-white/10">
      {TIERS.map(({ label, badge, caption }) => (
        <div key={label} className="flex flex-col items-center gap-0.5">
          <span className={`text-xs font-semibold px-2 py-0.5 rounded border ${badge}`}>
            {label}
          </span>
          <span className="text-[10px] text-white/60 text-center">{caption}</span>
        </div>
      ))}
    </div>
  );
